...
class CFT_v6(nn.Module):
    ...
    def __init__(self, cfg: dict):
        ...
        # GAP 之后的 LayerNorm（sum 聚合后数值范围大，需要归一化）
        self.pre_head_norm = nn.LayerNorm(self.F_token)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 3. GAP 沿 H 轴（论文 Section 2.1："sum up the harmonic features"）
        out = S.sum(dim=-1)             # (B, T, 48)
        out = self.pre_head_norm(out)   # LayerNorm 稳定数值
...
class CFTLoss(nn.Module):
    ...
    def forward(self, onset_pred, frame_pred, offset_pred,
        ...
        frame_loss  = F_func.binary_cross_entropy_with_logits(
            ...
            pos_weight=torch.tensor(self.frame_pos_weight, device=frame_pred.device))
        offset_loss = F_func.binary_cross_entropy_with_logits(
            offset_pred, offset_label,
            pos_weight=torch.tensor(self.offset_pos_weight, device=offset_pred.device))

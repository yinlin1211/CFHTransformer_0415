...
修改说明：
  ...
  [修改2] 在 run/<timestamp>_COnP/ 下额外记录 test_monitor.txt
          每 5 个 epoch（以及第 1 个 epoch）写入一次指标，便于画图
  [修改3] 保持 v6_0415 原有 config.yaml 与外部 splits_dir 兼容，不改数据路径
"""
...
def main():
    ...
    # 初始化 test_monitor.txt 表头
    test_log_path = run_dir / 'test_monitor.txt'
    if not test_log_path.exists():
        with open(test_log_path, 'w') as tf:
            tf.write("epoch\ttrain_loss\tval_loss\tCOn_f1\tCOnP_f1\tCOnPOff_f1\tlr\n")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        # 每 5 个 epoch 记录一次 test_monitor.txt，便于后续画图
        if epoch % 5 == 0 or epoch == 1:

...
def estimate_frame_end(frame_curve, start, stop, frame_thresh, max_gap):
    """Use the frame branch as fallback when no reliable offset peak is found."""
    last_active = start
    gap = 0
...
def frames_to_notes(frame_pred, onset_pred, hop_length, sample_rate,
                    ...
                    offset_pred=None, offset_thresh=None, max_gap=2):
    """帧级预测 → 音符列表。传入 offset_pred 时优先用 offset peak 定结束点。"""
    frame_time = hop_length / sample_rate
    T, P = frame_pred.shape
    intervals = []
    pitches = []
    use_offset = offset_pred is not None and offset_thresh is not None
    ...
    for p in range(P):
        ...
        if use_offset:
            ...
            offset_frames = np.zeros(0, dtype=np.int64)
            ...
            for i, f_on in enumerate(onset_frames):
                ...
                if len(valid_offsets) > 0:
                    ...
                    end_frame = estimate_frame_end(
                        frame_pred[:, p], f_on, search_stop, frame_thresh, max_gap
                    )

                if end_frame - f_on >= min_note_len:
                    intervals.append([f_on * frame_time, end_frame * frame_time])
                    pitches.append(float(midi))
...
def validate_full_song(model, val_dataset, criterion, device, hop_length, sample_rate,
                       ...
                       infer_chunk=256, gt_annotations=None):
    """
    全曲验证。
    ...
    with torch.no_grad():
        for idx in range(len(val_dataset)):
            ...
            offset_sig_list.append(offset_sig.mean())

            # 预测音符
            pred_intervals, pred_pitches = frames_to_notes(
                frame_sig, onset_sig, hop_length, sample_rate,
                onset_thresh, frame_thresh,
                offset_pred=offset_sig, offset_thresh=offset_thresh
            )
    ...
    avg_offset_sig = float(np.mean(offset_sig_list)) if offset_sig_list else 0.0

    return (
        avg_loss, avg_con_f1, avg_conp_f1, avg_conpoff_f1,
        avg_onset_sig, avg_frame_sig, avg_offset_sig
    )
...
def find_best_threshold(model, val_dataset, criterion, device, hop_length, sample_rate,
    ...
    offset_thresholds = [0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50]
    best_offset_key = (0.0, 0.0, 0.0)

    for ot in onset_thresholds:
        for ft in frame_thresholds:
            conp_list = []
            for frame_sig, onset_sig, offset_sig, ref_intervals, ref_pitches in preds:
                pred_intervals, pred_pitches = frames_to_notes(
    ...
    for off in offset_thresholds:
        ...
        for frame_sig, onset_sig, offset_sig, ref_intervals, ref_pitches in preds:
            pred_intervals, pred_pitches = frames_to_notes(
                frame_sig, onset_sig, hop_length, sample_rate,
                best_ot, best_ft, offset_pred=offset_sig, offset_thresh=off
            )
            if len(ref_intervals) == 0:
                continue
            ...
            if conpoff is not None:
                con_list.append(con)
                conp_list.append(conp)
                conpoff_list.append(conpoff)
        if conpoff_list:
            avg_conpoff = np.mean(conpoff_list)
            avg_conp = np.mean(conp_list)
            avg_con = np.mean(con_list)
            offset_key = (avg_conpoff, avg_conp, avg_con)
            if offset_key > best_offset_key:
                best_offset_key = offset_key
                best_conpoff = avg_conpoff
                best_off = off

    logger.info(
        f"  Threshold search: best on={best_ot:.2f}, fr={best_ft:.2f}, "
        f"off={best_off:.2f}, COnP_f1={best_conp:.4f}, COnPOff_f1={best_conpoff:.4f}"
    )
    return best_ot, best_ft, best_off
...
def main():
    ...
    if args.resume and Path(args.resume).exists():
        ...
        best_offset_thresh = ckpt.get('best_offset_thresh', 0.3)
        logger.info(
            f"Resumed from epoch {ckpt['epoch']}, best_COnP_f1={best_conp_f1:.4f}, "
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        val_loss, con_f1, conp_f1, conpoff_f1, onset_sig, frame_sig, offset_sig = validate_full_song(
            model, val_dataset, criterion, device, hop_length, sample_rate,
        ...
        logger.info(
            ...
            f"sig_onset={onset_sig:.4f} sig_frame={frame_sig:.4f} sig_offset={offset_sig:.4f} | "
            f"thresh(on={best_onset_thresh:.2f},fr={best_frame_thresh:.2f},off={best_offset_thresh:.2f}) | "
            f"lr={lr:.2e}"
        )
        ...
        if writer:
            ...
            writer.add_scalar('Threshold/onset', best_onset_thresh, epoch)
            writer.add_scalar('Threshold/frame', best_frame_thresh, epoch)
            writer.add_scalar('Threshold/offset', best_offset_thresh, epoch)
        ...
        # 另外在 epoch > 20 后，只要出现新的 best model，也额外评估一次。
        test_monitor_triggers = []
        if epoch % 40 == 0 or epoch == 1:
            test_monitor_triggers.append('periodic_40')
        if epoch > 20 and is_best_epoch:
            test_monitor_triggers.append('best_after20')

        if test_monitor_triggers:
            test_mon_loss, test_con, test_conp, test_conpoff, _, _, _ = validate_full_song(

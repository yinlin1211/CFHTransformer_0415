...
class FHTransformer(nn.Module):
    ...
    def forward(self, S: torch.Tensor) -> torch.Tensor:
        ...
        if T <= self.temporal_embed.num_embeddings:
            ...
            t_emb = self.temporal_embed(t_idx)
        else:

            t_emb_all = self.temporal_embed.weight
            t_emb = F_func.interpolate(
                t_emb_all.unsqueeze(0).transpose(1, 2),
...
class CFT_v6(nn.Module):
    ...
    def __init__(self, cfg: dict):
        ...
        assert self.H % self.nhead_fh == 0,            f"H={self.H} 必须能被 nhead_fh={self.nhead_fh} 整除"
        assert self.H % self.nhead_ht == 0,            f"H={self.H} 必须能被 nhead_ht={self.nhead_ht} 整除"
        assert self.num_pitches % self.nhead_tf == 0,            f"num_pitches={self.num_pitches} 必须能被 nhead_tf={self.nhead_tf} 整除"
...
class CFTLoss(nn.Module):
    ...
    def forward(self, onset_pred, frame_pred, offset_pred,
        ...
        onset_loss  = F_func.binary_cross_entropy_with_logits(
            ...
            pos_weight=torch.tensor(self.onset_pos_weight, device=onset_pred.device, dtype=onset_pred.dtype))
        frame_loss  = F_func.binary_cross_entropy_with_logits(
            frame_pred, frame_label,
            pos_weight=torch.tensor(self.frame_pos_weight, device=frame_pred.device, dtype=frame_pred.dtype))
        offset_loss = F_func.binary_cross_entropy_with_logits(
            offset_pred, offset_label,
            pos_weight=torch.tensor(self.offset_pos_weight, device=offset_pred.device, dtype=offset_pred.dtype))
